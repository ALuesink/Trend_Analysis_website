	...
	for run_path in runs_path:
		...
		if run not in run_in_db and "TEST" not in run and "illumina" not in run and "rsync" not in run and "_2" not in run[-2:]:
			try:
				...
				insert_Run = Run.insert().values(Run=str(run), Cluster_Raw=run_stats[0], Cluster_PF=run_stats[1], Yield_Mbases=run_stats[2], Seq_ID=seq_id, Date=date, asDate=as_Date, Sequencer = sequencer)
