...
function(input, output, session){
  ...
  query_run_table <- function(columns){
    con <- dbConnect(RMySQL::MySQL(), group="trendngs")
    query <- sprintf("SELECT %s, Run, Sequencer FROM Run JOIN Sample_Sequencer ON Run.Run_ID = Sample_Sequencer.Run_ID",columns)
    response <- dbSendQuery(con, query)
    results <- dbFetch(response, n=-1)
    dbClearResult(response)
    dbDisconnect(con)
  ...
  query_lane_table <- function(columns){
    con <- dbConnect(RMySQL::MySQL(), group="trendngs")
    query <- sprintf("SELECT %s,Run, Sequencer FROM Run JOIN Run_per_Lane ON Run.Run_ID = Run_per_Lane.Run_ID",columns)
    response <- dbSendQuery(con, query)
    results <- dbFetch(response, n=-1)
    dbClearResult(response)
    dbDisconnect(con)
  ...
  query_processed_table <- function(columns){
    con <- dbConnect(RMySQL::MySQL(), group="trendngs")
    query <- sprintf("SELECT %s, Run, Sequencer FROM Run JOIN Sample_Processed ON Run.Run_ID = Sample_Processed.Run_ID", columns)
    response <- dbSendQuery(con, query)
    results <- dbFetch(response, n=-1)
    dbClearResult(response)
    dbDisconnect(con)
  ...
  brush_run_plot <-function(x_var,y_var){
    ...
    new_data <- sub_data[which(sub_data$plotID >= brush_xmin & sub_data$plotID <= brush_xmax & sub_data[[y_var]] >= brush_ymin & sub_data[[y_var]] <= brush_ymax), c("Lane","Sample_name","Project","PCT_Q30_bases","PCT_PF_Clusters", "Mean_Quality_Score", "PCT_one_mismatch_barcode", "Run", "Sequencer")]
  ...
  output$Run_Q30_brushed <- DT::renderDataTable({
    brush_run_plot("Lane, Sample_name, Project, PCT_PF_Clusters,Run,Mean_Quality_Score,PCT_one_mismatch_barcode","PCT_Q30_bases")
  })
  output$Run_MQS_brushed <- DT::renderDataTable({
    brush_run_plot("Lane, Sample_name, Project, PCT_PF_Clusters,PCT_Q30_bases,Run,PCT_one_mismatch_barcode","Mean_Quality_Score")
  })
  output$Run_Bar_brushed <- DT::renderDataTable({
    brush_run_plot("Lane, Sample_name, Project,PCT_PF_Clusters,PCT_Q30_bases,Mean_Quality_Score,Run", "PCT_one_mismatch_barcode")
  })
